...
df_filtered <- na.omit(df_dates) |> 
  dplyr::filter(onset_to_hosp >= 1)

cres <- nfidd_sample(cmod,
  data = list(
    n = nrow(df_filtered),
    onset_to_hosp = df_filtered$onset_to_hosp
  )
)

df_dates |>
  drop_na(onset_to_hosp) |>
  summarise(mean = mean(onset_to_hosp),
            min = min(onset_to_hosp),
            max = max(onset_to_hosp)) # 
```
...
# How epidemic dynamics affect bias severity {#how-epidemic-dynamics-affect-bias-severity}
...
-   The solutions we introduced for addressing censoring and truncation are only some possible ones for the censoring problem. Other solutions reduce the biases from estimation even further. For an overview, see the review by [@parkEstimatingEpidemiologicalDelay2024].
